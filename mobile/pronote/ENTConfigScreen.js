      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {showForm && cfg && (
          <View style={[styles.form, { borderColor: cfg.color + '44' }]}>
            ...
            {cfg.needsUrl && (
              <TouchableOpacity style={styles.webViewBtn} onPress={openPronoteWebView}>
                <Text style={styles.webViewBtnText}>🌐 Ouvrir Pronote / EduConnect</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
        ...
        <Modal visible={webViewVisible} animationType="slide" onRequestClose={() => setWebViewVisible(false)}>
          <View style={styles.webViewContainer}>
            <View style={styles.webViewHeader}>
              <Text style={styles.webViewTitle}>Connexion Pronote</Text>
              <TouchableOpacity onPress={() => setWebViewVisible(false)}>
                <Text style={styles.webViewClose}>Fermer</Text>
              </TouchableOpacity>
            </View>
            <WebView
              source={{ uri: form.schoolUrl }}
              sharedCookiesEnabled
              thirdPartyCookiesEnabled
              javaScriptEnabled
              domStorageEnabled
              startInLoadingState
              renderLoading={() => <ActivityIndicator style={styles.webViewLoading} color="#5B6BF5" size="large" />}
            />
          </View>
        </Modal>
  webViewBtn: { marginTop: 12, borderWidth: 1, borderColor: '#5B6BF5', borderRadius: 10, padding: 13, alignItems: 'center' },
  webViewBtnText: { color: '#AAB4FF', fontWeight: '700' },
  webViewContainer: { flex: 1, backgroundColor: '#0a0a12' },
  webViewHeader: { height: 56, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#29293a' },
  webViewTitle: { color: '#fff', fontSize: 17, fontWeight: '700' },
  webViewClose: { color: '#AAB4FF', fontWeight: '700' },
