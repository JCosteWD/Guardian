...
describe('Auth', () => {
  ...
  test('login démo → 200 avec compte test conservé', async () => {
    process.env.DEMO_MODE = 'true';
    const res = await request(app).post('/api/auth/login').send({ email: 'demo@guardian.com', password: 'Password123' });
    expect(res.status).toBe(200);
    expect(res.body.parent.email).toBe('demo@guardian.com');
    expect(res.body.parent.isDemo).toBe(true);
  });

  test('login démo avec mauvais mdp → 401', async () => {
    process.env.DEMO_MODE = 'true';
    const res = await request(app).post('/api/auth/login').send({ email: 'demo@guardian.com', password: 'wrong' });
    expect(res.status).toBe(401);
  });
