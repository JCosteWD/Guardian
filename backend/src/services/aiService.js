...
logger.debug('[AI Service] mockMode:', mockMode, 'NODE_ENV:', process.env.NODE_ENV, 'DEMO_MODE:', process.env.DEMO_MODE);
...
const getMockAIResponse = (message, childName, childAge, educationalLevel, conversationHistory = []) => {
  ...
  // Check if this is a response to a previous question (simple context tracking)
  const lastMessage = conversationHistory[conversationHistory.length - 2]?.content?.toLowerCase() || '';
  
  // ── RESPONSE VALIDATION (highest priority) ─────────────────────────────────────
  // Check if answering a fraction question
  ...
  // Check if answering a math equation question
  ...
  // ── SPECIFIC TOPICS (check BEFORE general categories) ─────────────────────
  ...
  // History/Geography specific topics
  ...
  // ── GENERAL CATEGORIES (check AFTER specific topics) ───────────────────────
...
const buildChildContext = async (childId) => {
  const [childRes, gradesRes, quotaRes, behaviorRes] = await Promise.all([
    query(
      `SELECT c.first_name, c.age, c.educational_level, c.ai_persona_name, c.ai_tone,
  ...
  const educationalLevel = child.educational_level || getLevelFromAge(child.age);
...
exports.chat = async (req, res) => {
  ...
  const { message, sessionId, conversationHistory: clientHistory } = req.body;

  logger.debug('[AI Chat] Request received:', { childId, message: message?.substring(0, 50), mockMode });
  ...
  try {
    ...
    if (req.child?.subscription_plan && req.child.subscription_plan !== 'premium') {
      return res.status(403).json({ error: 'Quiz disponible avec le plan Premium', code: 'UPGRADE_REQUIRED' });
    }

    // Always use mock mode in non-production environments and demo mode.
    if (process.env.NODE_ENV !== 'production' || process.env.DEMO_MODE === 'true' || !process.env.ANTHROPIC_API_KEY) {
      childName = req.child?.first_name || 'Enfant';
      childAge = req.child?.age || 10;
      educationalLevel = req.child?.educational_level || 'CM2';
      logger.debug('[AI Chat] Using mock mode with defaults, level:', educationalLevel);

      let aiResponse;
      let quizRequested = null;
      let mood = null;
      let remainingMins = 120;

      try {
        const liveQuota = req.child?.id ? await quota.get(req.child.id) : null;
        if (liveQuota) {
          const total = liveQuota.baseLimitMins + liveQuota.bonusMins - liveQuota.penaltyMins;
          remainingMins = Math.max(0, total - liveQuota.usedMins);
        }
      } catch (err) {
        logger.warn('[AI Chat] Unable to resolve quota for mock mode', err.message);
      }

      // Use mock responses with educational level and conversation history
      logger.debug('[AI Chat] Generating mock response with history length:', conversationHistory.length);
      aiResponse = getMockAIResponse(message, childName, childAge, educationalLevel, conversationHistory);
      logger.debug('[AI Chat] Mock response generated:', aiResponse?.substring(0, 100));
      ...
      logger.debug('[AI Chat] Sending response:', { aiResponse: aiResponse?.substring(0, 50), quizRequested, mood, remainingMins });
    ...
    // Production mode with Anthropic API
    ...
    await query(
      `INSERT INTO activity_events (child_id, event_type, payload)
       VALUES ($1, 'ai_chat', $2)`,
      [childId, JSON.stringify({ message: message.substring(0, 100), mood, educationalLevel })]
    );
...
exports.generateQuiz = async (req, res) => {
  ...
  if (mockMode) {
    return res.status(503).json({ error: 'Quiz non disponible en mode développement sans clé API' });
  }
  ...
  try {
    ...
    if (passed) {
      ...
      try {
        ...
        // notificationService.sendToParent(child.rows[0].parent_id, {...});
      } catch (err) {
    ...
    let aiMessage = passed ? 'Bravo ! Continue comme ça !' : 'Ne lâche rien, tu vas y arriver !';
...
Génère UN message d'encouragement court (2-3 phrases max), adapté à ${req.child.age} ans.`;
      ...
      try {
        ...
        logger.warn('Failed to generate AI encouragement message:', err.message);
      }
    }
...
exports.generateWeeklyReport = async (parentId, childId) => {
  if (mockMode) {
    return 'Rapport hebdomadaire non disponible en mode développement sans clé API';
  }
