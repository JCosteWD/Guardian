...
exports.createChild = async (req, res) => {
  ...
  try {
    // MODE DÉMO FORCÉ - Création d'enfant simulée
    if (process.env.DEMO_MODE === 'true' || process.env.NODE_ENV === 'development') {
      logger.warn('Mode démo activé - Simulation création enfant');
      ...
      const demoChild = {
        ...
        created_at: new Date(),
        last_seen: null,
        pin_hash: pin ? 'hashed_pin_demo' : null
      };

      logger.info(`Demo child created: ${childId} for parent ${req.user.id}`);
      return res.status(201).json({
        child: demoChild,
        pairingCode,
        message: `Profil créé (mode démo). Code d'appairage : ${pairingCode}`,
      });
    }
